 ...
 * - 뒤쪽 변형 suffix (예: "[1m]" = 1M 컨텍스트 변형) 제거 — 1M 컨텍스트도 standard 단가라 동일 모델로 합산
 * - 뒤쪽 날짜 suffix (YYYYMMDD 8자리) 제거
 * - 그 외에는 원본 유지
 */
export function normalizeModel(rawModel) {
  if (!rawModel || typeof rawModel !== "string") return ""
  // 1) "claude-opus-4-8[1m]" 같은 끝의 대괄호 변형 suffix 제거
  // 2) "-20251001" 같은 끝의 날짜 suffix 제거
  return rawModel.replace(/\[[^\]]*\]$/, "").replace(/-\d{8}$/, "")
}
...
function resolveRates(normalizedModel) {
  ...
  if (!warnedUnknownModels.has(normalizedModel)) {
    ...
    console.warn(
      `[usage-parser] unknown model: ${normalizedModel} — fallback(${fallbackKey}) 단가 적용`,
    )
  }
  return {
    unit: pricing.unitTokens || 1_000_000,
...
export function parseUsageEvent(entry) {
  ...
  const cacheWrite1h = has1h ? creation.ephemeral_1h_input_tokens : has5m ? 0 : totalCreation
...
/**
 * CC는 API 응답 1개를 content block마다 한 줄씩 쓰고, 줄마다 같은 message.usage를 복사한다.
 * 같은 응답의 줄은 파일 안에서 항상 연달아 나온다 (2026-09-23 실측: usage 19,967줄 = 응답 8,875개,
 * 비연속 반복·파일 간 중복 0건). → 직전 usage 줄과 message.id가 같으면 이미 센 응답.
 * last는 파일별 상태 { id } — 커서 이전 줄에도 호출해야 증분 경계에서 이어진다.
 */
export function isRepeatedUsage(obj, last) {
  const id = obj.message && obj.message.usage ? obj.message.id : null
  if (!id) return false
